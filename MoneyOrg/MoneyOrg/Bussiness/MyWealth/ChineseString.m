...
{
    NSMutableArray *tempArray = [self ReturnSortChineseArrar:dicArr];
    ...
    for (NSString* object in tempArray) {
        ...
        NSDictionary * dic =  ((ChineseString*)object).dic;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        chineseString.string=[NSString stringWithString:[[stringArr objectAtIndex:i]objectForKey:@"name"]];
        chineseString.dic = [stringArr objectAtIndex:i];
